...
class Command(BaseCommand):
    help = 'Process all PDFs in Page3 folder and extract signature data to database'
    ...
    def handle(self, *args, **options):
        ...
        PDFFilePath = os.path.join(settings.BASE_DIR, "output", folder, f"{folder}_Output", "Page3")
        ...
        log_file = os.path.join(log_folder, f"page3_{today}.log")

        logger = logging.getLogger('page3')
        ...
        def process_single_pdf(pdf_path, file_name, file_record, is_retry=False):
            try:
                ...
                headers = ["LAN_ID", "Applicant_Signature"]
                ...
                if not signed_by:
                    ...
                    for line in lines:
                        ...
                        if "signed by:" in lower:
                            signed_by = line_clean.split(":", 1)[1].strip()
                            break

                extracted_data["Applicant_Signature"] = signed_by
                ...
                logger.error(f"PDF reading/processing error during table extraction: {error_details}")
                return None, None
        ...
        try:
            ...
            self.stdout.write("INFO - Starting Page3 processing...")
            ...
            logger.info(f"Page3 complete. Total archived: {total_archived}, Time: {minutes}m {seconds}s")
